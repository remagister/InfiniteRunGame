...
public:
	...
	// fulltex - определяет тип разбиения спрайта на кадры. либо до конца ряда, либо до конца изображения
	...
	/// <summary>
	/// Обновляет изображение анимации с указанным временем
	/// </summary>
	/// <param name="dt">Разница во времени между игровыми кадрами.</param>
...
private:
	...
	float _dt;
	int _frame;
	std::vector<sf::IntRect> _frames;	// массив фреймов
	std::vector<sf::IntRect> _flip_frames;
	// public fields

	/// <summary>
	/// Задает или получает количество кадров в анимации, идущих слева направо и сверху вниз (при превышении размеров спрайта)
	/// </summary>
public:
	int Frames;
	/// <summary>
	/// Задает или получает размер прямоугольного кадра анимации, вид которого перемещается по спрайту
	/// </summary>
	sf::IntRect FrameRect;
	/// <summary>
	/// Задает или получает значение скорости воспроизведения анимации (кадры в секунду)
	/// </summary>
	float Speed;
};


// класс, представляющий средства управления различным количеством связанных анимаций
// может быть использован в качестве отдельного графического элемента
